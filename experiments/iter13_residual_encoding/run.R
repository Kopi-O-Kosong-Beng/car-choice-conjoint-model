...
#
# Emits: oof_xgb_bagRes.rds / test_xgb_bagRes.rds
...
PAR <- list(eta = 0.05, max_depth = 6, min_child_weight = 10,
...
saveRDS(data.table(No = nos_tr, p1 = Pa[,1], p2 = Pa[,2], p3 = Pa[,3], p4 = Pa[,4]),
        "model/artifacts/oof_xgb_bagRes.rds")
...
saveRDS(data.table(No = nos_te, p1 = Pte[,1], p2 = Pte[,2], p3 = Pte[,3], p4 = Pte[,4]),
        "model/artifacts/test_xgb_bagRes.rds")
