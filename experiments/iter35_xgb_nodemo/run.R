...
NODEMO <- Sys.getenv("XGB_NODEMO", "0") == "1"
cat("XGB_NODEMO (drop the 15 demographic features):", NODEMO, "
")
OUTDIR <- "experiments/iter35_xgb_nodemo"
...
feat <- c(ATTRS, paste0(ATTRS, "_c"), "richness","lvlsum","price_rank","price_min_rival",
          "price_task_mean","rich_task_mean","Task", paste0("alt", 1:4),
          if (NODEMO) character(0) else demo, ENC_COLS)
...
if (JOB == "combine") {
  ...
  NM <- paste0(if (CFG == "mono") "xgb_monobag" else "xgb_lw2bag", if (NODEMO) "_nod" else "")
